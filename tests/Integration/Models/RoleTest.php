...
class RoleTest extends IntegrationTestCase
{
    ...
    public function test_get_all_with_user_count_ordered_by_name(): void
    {
        ...
        $this->assertSame(['Administrator', 'Auditor', 'Editor'], $names);
    }
    ...
    public function test_get_all_active_returns_only_active_roles(): void
    {
        ...
        $activeNames = array_column($roles, 'name');
        $this->assertContains('Administrator', $activeNames);
        $this->assertContains('Editor', $activeNames);
    }
    ...
    {
        // Checking 'Editor' (id=2) while excluding id=2 should return false
        $this->assertFalse($this->model->nameExists('Editor', 2));
    }
    ...
    {
        $result = $this->model->update(2, ['name' => 'Senior Editor', 'description' => 'Updated']);
        $this->assertTrue($result);
    ...
    {
        // Try to rename role 2 (Editor) to 'Auditor' (already taken by role 3)
        $result = $this->model->update(2, ['name' => 'Auditor', 'description' => '']);
        $this->assertFalse($result);
    }
    ...
    public function test_update_allows_same_name_for_own_id(): void
    {
        ...
        $result = $this->model->update(2, ['name' => 'Editor', 'description' => 'New desc']);
        $this->assertTrue($result);
    }
    ...
    {
        $result = $this->model->updateStatus(2, 0);
        $this->assertTrue($result);
    ...
    {
        $result = $this->model->updateStatus(3, 1);
        $this->assertTrue($result);

        $role = $this->model->getById(3);
        $this->assertSame(1, (int) $role['status']);
    }
    ...
    {
        // Auditor (id=3) has no users assigned in the seed
        $this->assertSame(0, $this->model->countUsers(3));
    }

    public function test_count_users_returns_correct_count_after_assignment(): void
    {
        // Assign role 3 (Auditor) to user 2 directly via PDO
        self::$pdo->exec('UPDATE users SET role_id = 3 WHERE id = 2');

        $this->assertSame(1, $this->model->countUsers(3));
    }
}
